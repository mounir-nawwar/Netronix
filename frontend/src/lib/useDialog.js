...
export default function useDialog({ open, onClose, lockScroll = false }) {
    ...
    useEffect(() => {
        ...
        const onKeyDown = (event) => {
            ...
            // Every Tab is handled here and focus is moved explicitly.
            //
            // The obvious implementation only intercepts the two edges — wrap
            // at the last element going forward, at the first going back — and
            // lets the browser do the rest. That relies on the browser's own
            // tab order agreeing with `querySelectorAll` order for everything
            // in between, which is not guaranteed: a positive `tabindex`, a
            // portal, or a `motion` element that has not settled will disagree,
            // and when they disagree focus leaves the dialog on an *interior*
            // Tab, where an edge-only trap is not watching. Moving by index
            // makes the trap independent of all of that.
            event.preventDefault()
